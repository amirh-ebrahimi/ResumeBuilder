...
if (isset($_POST["jobseeker"]) && jobseekerFieldsAreFilled() && jobseekerValidation()) {
    ...
    if (!empty(trim($_POST["job1-title"]))) { // if the job is entered then...
        ...
        $job1 = [$_POST["job1-title"], $_POST["job1-start"], $_POST["job1-end"], $_POST["job1-reason"]]; // enter all the details of job in an array
    }else{
    ...
    if (!empty(trim($_POST["job2-title"]))) { // if the job is entered then...
        ...
        $job2 = [$_POST["job2-title"], $_POST["job2-start"], $_POST["job2-end"], $_POST["job2-reason"]]; // enter all the details of job in an array
    }else{
    ...
    if (!empty(trim($_POST["job3-title"]))) { // if the job is entered then...
        ...
        $job3 = [$_POST["job3-title"], $_POST["job3-start"], $_POST["job3-end"], $_POST["job3-reason"]]; // enter all the details of job in an array
    }else{
    ...
    $jobseeker_id = newJobseeker($username, $password_hash, $name, $family, $nationality, $gender, $birth_date, $birth_place, $email, $phone, $details,$edu_id);
